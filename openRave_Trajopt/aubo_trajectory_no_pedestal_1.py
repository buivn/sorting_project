...
def traj_test():
    ...
    # Load OpenRave (simulation) environment 
    env = Environment()

    if (debug):
        env.SetDebugLevel(1)
        env.SetViewer('qtcoin')
        env.Load('/home/buivn/bui_IROS2020/src/environment.xml') # load a scene with the robot inside
        # draw some coordinates system
    ...
    # print("just check the dimension of a pcd file ", len(rgbfloats[0]))
    # rgb0 = np.ndarray(buffer=rgbfloats.copy(),shape=(480*640,4),dtype='uint8')
    rgb0 = np.ndarray(buffer=rgbfloats.copy(),shape=(len(rgbfloats),4),dtype='uint8')
    ...
    # # Simplify the mesh by some functions from trajopt
   ...
   # Target in point cloud coordinate
    ...
    request = {        
        ...
        "constraints" : [
        {
            "type" : "pose",  # joint-space target
            "name" : "final_pose",
            "params" : {
                "pos_coeffs" : [1,1,1],
                "rot_coeffs" : [1,1,1],
                "xyz" : xyz_target,   # position of the destination
                "wxyz" : grasp_quat_target,   # orientation of the destination
                "link" : "wrist3_Link",
            },
        }
        ],
    ...
    # set up trajopt optimization 
    s = json.dumps(request) # convert dictionary into json-formatted string
    trajoptpy.SetInteractive(args.interactive)
    prob = trajoptpy.ConstructProblem(s, env) # create object that stores optimization problem
    
    # get the set of points on the trajectory
